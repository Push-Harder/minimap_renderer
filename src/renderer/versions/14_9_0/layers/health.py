...
class LayerHealthBase(LayerBase):
    ...
    def draw(self, game_time: int, image: Image.Image):
        if self._didFail:
            return
        try:
            self._draw(game_time, image)
        except Exception as e:
            print(f"Exception encountered rendering Health layer, terminating draw() for this layer. {e}")
            self._didFail = True

    def _draw(self, game_time: int, image: Image.Image):
